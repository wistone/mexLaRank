...
/*
** OUTPUT: one per class of the training set, keep tracks of support vectors and their beta coefficients
*/
class LaRankOutput
{
public:
  ...
  // Set beta and rtoi
  void set_beta_and_rtoi (double* ptr, int dim_sv) const
  {
    int *r2i = larank_kcache_r2i (kernel, l);
    LaFVector rtoi;
    for (int r = 0; r < l; r++)
...
public:
  ...
  // PREDICTION FUNCTION: main function in la_rank_classify
  virtual int predict_with_score (int x_id, double* score_max)
  {
    int res = -1;
    score_max[0] = -DBL_MAX;
    for (outputhash_t::iterator it = outputs.begin (); it != outputs.end ();++it)
  ...
  char *model_to_matlab_structure(mxArray *plhs[])
  {
    int i, j, n;
    double *ptr; double *ptr_beta; double *ptr_rtoi;
    mxArray *return_model, **rhs;
    int out_id = 0;

    rhs = (mxArray **)mxMalloc(sizeof(mxArray *)*NUM_OF_RETURN_FIELD);

    // a vector for output label and sv number as in model.txt
    int nb_output = 0;
    for (outputhash_t::const_iterator it = outputs.begin (); it != outputs.end (); ++it)
    {
      nb_output ++;
    }
    rhs[out_id] = mxCreateDoubleMatrix(nb_output * 2, 1, mxREAL);
    ...
    // a matrix capture the beta and rtoi vectors in all classes
    mxArray *pprhs[1], *pplhs[1];
    rhs[out_id] = mxCreateDoubleMatrix(dim_sv, nb_output * 2, mxREAL);
    ptr = mxGetPr(rhs[out_id]);
    pprhs[0] = rhs[out_id];
    //out_id++;
    ...
    for (outputhash_t::const_iterator it = outputs.begin (); it != outputs.end (); ++it)
    {
      it->second.set_beta_and_rtoi(ptr, dim_sv);
      ptr = ptr + dim_sv * 2;
    }

    if(mexCallMATLAB(1, pplhs, 1, pprhs, "transpose")) 
    {
      mexPrintf("cannot transpose SV matrix\n");
      return NULL;
    }
    rhs[out_id] = pplhs[0];
